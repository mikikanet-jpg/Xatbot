...
public class xatbot {
    ...
    private static final String GREEN = "\u001B[32m";   // Usuari
    private static final String BLUE = "\u001B[34m";    // Assistent
    private static final String YELLOW = "\u001B[33m";  // Sistema
    private static final String RED = "\u001B[31m";     // Errors
    ...
    public xatbot() {
        ...
        if (apiKey == null || apiKey.isEmpty()) {
            System.out.println(RED + "Error: No s'ha trobat la variable d'entorn OPENAI_API_KEY." + RESET);
            System.exit(1);
        }
    ...
    public void start() {
        ...
        System.out.println(YELLOW + "--- Xat amb ChatGPT ---" + RESET);
        System.out.println(YELLOW + "Escriu /help per veure les opcions o /exit per sortir." + RESET);
        ...
        while (true) {
            ...
            if (input.startsWith("/role ")) {
                ...
                setSystemRole(role);
                System.out.println(YELLOW + "[SISTEMA] Rol canviat: " + role + RESET);
                continue;
            }

            if (input.startsWith("/model ")) {
                model = input.substring(7).trim();
                System.out.println(YELLOW + "[SISTEMA] Model canviat a: " + model + RESET);
                continue;
            }

            // Afegeix la frase de l'usuari a la conversa
            conversation.add(Map.of("role", "user", "content", input));

            try {
                String response = getChatGPTResponse();
                System.out.println(BLUE + "Assistent: " + response + RESET);
                conversation.add(Map.of("role", "assistant", "content", response));
            } catch (Exception e) {
                System.out.println(RED + "[ERROR] No s'ha pogut obtenir resposta: " + e.getMessage() + RESET);
            }
        }
        sc.close();
        System.out.println(YELLOW + "Sessió finalitzada." + RESET);
    }
